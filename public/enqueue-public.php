...
function pdr_enqueue_scripts() {
    wp_enqueue_style('pdr-style', plugins_url('/public/css/style.css', PDR_MAIN_FILE));
    ...
    wp_enqueue_script('google-maps', 'https://maps.googleapis.com/maps/api/js?key=' . $google_maps_api_key . '&libraries=places&callback=initAutocomplete', array('pdr-script'), null, true);
     */
    ...
    wp_enqueue_script('pdr-script', plugins_url('/public/js/script.js', PDR_MAIN_FILE), array('jquery'), '', true);
...
add_action('wp_enqueue_scripts', 'pdr_enqueue_scripts');
