...
class ReferralHub_Admin {
    ...
    public function __construct() {
        ...
        add_filter('views_edit-rhb_service', [$this, 'adjust_service_views']);
    }

    // Função para restringir a visualização de serviços no admin
    public function restrict_service_view($query) {
        if (is_admin() && $query->is_main_query() && $query->get('post_type') === 'rhb_service') {
    ...
    public function adjust_service_views($views) {
        if (get_post_type() === 'rhb_service' && current_user_can('service_provider') && !current_user_can('administrator')) {
            // Remover a aba "All"
            if (isset($views['all'])) {
                unset($views['all']);
            }
